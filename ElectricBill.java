...
public class ElectricBill {
	...
	public static double calcBusiness() {
		...
		if (userEnergy <= 1200) {
			...
			return((userEnergy * energyChargeb4) + flatCharge +(premChannel * 47)) * busSalesTax;
		}else {// return if energy is greater than 1200
			return ((1200*energyChargeb4)+((userEnergy - 1200) * energyChargeAfter) + flatCharge +(premChannel * 47)) * busSalesTax;
		}
	}
	...
	public static void menuDispley(double billing) {
		boolean x = false;
		displayBill(billing);
		// main while statement
		while (!x) {
			System.out.printf("%n%nI- Enroll in out installment plan%nV- View Change in consumption with previous bill%nE- Exit%nUser Choice: ");
